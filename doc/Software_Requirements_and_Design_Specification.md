...
doc/Software_Requirements_and_Design_Specification.md
...
# Chapter 6 — Debugger and Disassembler

---

## 6.1 Purpose

This chapter specifies the integrated debugging facilities provided by
the CHIP-8 Emulator.

The debugger enables the user to observe, inspect, and control the
execution of the virtual machine without modifying its behaviour.

The disassembler provides a human-readable representation of the loaded
program.
...
- provide complete visibility of the machine state,
- permit controlled execution,
- assist program analysis,
- simplify emulator verification.

The debugger shall never alter emulator behaviour except when executing
...
The debugger shall observe, but not modify, the virtual machine state
...
The debugger shall display the following information.
...
The debugger shall display the complete machine state.

---

## 6.4 Execution Control

The debugger shall support the following execution modes.
...
Single-step execution shall execute exactly one instruction.
...
Run mode shall execute instructions continuously until interrupted.

**REQ-DBG-604**

Pause shall preserve the complete machine state.

---

## 6.5 Disassembler

The disassembler converts machine instructions into symbolic assembly
language.

Disassembly shall use the same instruction decoding logic as execution.

The disassembler shall not implement an independent instruction decoder.
...
Execution and disassembly shall use a shared instruction decoder.

**REQ-DBG-606**

The disassembler shall produce deterministic output.

---

## 6.6 Instruction Highlighting

The debugger shall identify the instruction that will be executed next.
...
The instruction at the current program counter shall be visually
distinguished from all other instructions.

---

## 6.7 GUI Synchronization

Debugger information shall always correspond to the current machine
state.

The debugger shall never display partially executed instructions.
...
Debugger views shall be updated after every executed instruction.

**REQ-DBG-609**

All debugger views shall represent the same machine state.

---

## 6.8 Future Extensions

The debugger architecture shall permit future additions such as
...
These features are not part of Version 1.x.
...
Future debugger extensions shall not require redesign of the existing
architecture.
...
| ID | Description |
|----|-------------|
| REQ-DBG-600 | Debugger observes machine state |
| REQ-DBG-601 | Complete machine state visible |
| REQ-DBG-602 | Single-step executes one instruction |
| REQ-DBG-603 | Run executes continuously |
| REQ-DBG-604 | Pause preserves machine state |
| REQ-DBG-605 | Shared instruction decoder |
| REQ-DBG-606 | Deterministic disassembly |
| REQ-DBG-607 | Highlight current instruction |
| REQ-DBG-608 | Update after every instruction |
| REQ-DBG-609 | All debugger views synchronized |
| REQ-DBG-610 | Extensible debugger architecture |
