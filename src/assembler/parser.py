...
class Parser:
    ...
    def _parse_line(self) -> SourceLine:
        ...
        raise ParserError(
            f"Expected end of line. "
            f"Found '{token.value}' at "
    ...
    def _parse_operands(self) -> list[Expression]:
        ...
        while self._match(TokenType.COMMA):
            if self._check(TokenType.END_OF_LINE):
                token = self._current()
                raise ParserError(
                    f"Expected operand after comma at "
                    f"{token.location.line}:{token.location.column}."
                )
    ...
    def _parse_primary(self) -> Expression:
        ...
        if token.type == TokenType.NUMBER:
            ...
            try:
                ...
                raise ParserError(
                    f"Invalid numeric literal '{token.value}' at "
                    f"{token.location.line}:{token.location.column}."
                ) from error
        ...
        if token.type == TokenType.CHARACTER:
            ...
            if len(token.value) != 1:
                raise ParserError(
                    f"Invalid character literal at "
                    f"{token.location.line}:{token.location.column}."
                )
    ...
    def _expect(
        ...
        if not self._check(token_type):
            ...
            raise ParserError(
                f"{message} Found '{token.value}' at "
                f"{token.location.line}:{token.location.column}."
            )
